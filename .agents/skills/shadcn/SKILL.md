...
- **Use `asChild` (radix) or `render` (base) for custom triggers.** Check `base` field from `npx shadcn@latest info`. Note that React Aria (`aria`) is currently unsupported. → [base-vs-radix.md](./rules/base-vs-radix.md)
