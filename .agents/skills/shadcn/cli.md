...
> **IMPORTANT:** The CLI auto-detects the package manager from the project's lockfile; there is no `--package-manager` flag. Note that some valid flags (e.g., `--base`, `--no-reinstall`, CSS-variable, and pointer flags) might not be exhaustively listed below but are supported.
